...
## MVPで再現する範囲（暫定）
> 公式情報の確認後に確定する。現状は仮のMVP案。

- プレイヤー操作: 基本移動（前進/後退/旋回）と射撃（弾発射・クールダウン）。  
- 敵: 1 種類の敵タンク（HP/移動/射撃の基本パラメータのみ）。  
- ステージ: 1 ステージ（固定マップ、障害物配置、開始地点/ゴール or 敵全滅条件）。  
- ルール: HP、被弾、無敵時間、弾消滅などの基本ループ。  
- 最低限のUI: HP表示、ステージ開始/終了の状態遷移（リトライ含む）。  

## 次段階で追加する要素（暫定）
> MVP完了後に拡張する要素の整理。

- AI強化: 索敵、回り込み、遮蔽利用、味方連携など。  
- 複数ステージ: ステージ選択、難易度カーブ、勝利条件のバリエーション。  
- アイテム: 回復、弾種切替、速度アップなどの一時効果。  
- 演出強化: 爆発/ヒット演出、画面シェイク、サウンド/簡易BGM。  

## ロードマップ（依存関係と見積り）
> 1 人日 = 8h 想定の粗い見積り。要調整。

| フェーズ | タスク | 依存関係 | 見積り |
| --- | --- | --- | --- |
| 0 | MVP要件確定・仕様微修正 | なし | 0.5 人日 |
| 1 | プレイヤー移動/射撃の実装 | フェーズ0 | 1.0 人日 |
| 2 | 弾・当たり判定・ダメージの実装 | フェーズ1 | 1.0 人日 |
| 3 | 敵タンク（1種）行動・被弾実装 | フェーズ2 | 1.0 人日 |
| 4 | 1ステージ構築（マップ/障害物/勝敗条件） | フェーズ2 | 0.5 人日 |
| 5 | MVP UI/状態遷移（開始/終了/リトライ） | フェーズ1,4 | 0.5 人日 |
| 6 | AI強化（索敵/回り込み/遮蔽） | フェーズ3 | 2.0 人日 |
| 7 | 複数ステージ/難易度調整 | フェーズ4 | 1.5 人日 |
| 8 | アイテム/パワーアップ | フェーズ3 | 1.0 人日 |
| 9 | 演出強化（VFX/SFX/画面シェイク） | フェーズ2 | 1.0 人日 |
